  ...
  <!-- 내 홈으로 가기 -->
      ...
      <div style="height: 80vh; width: 75%; overflow:scroll;" class="p-5 m-2" id="border">
        ...
        <div class="d-flex justify-content-center">
          <router-link to="/movies/friends" class="link"><button class="btn btn-outline-secondarys fs-4 mx-4" style="width: 170px;">내 친구 관리</button></router-link>
          <router-link to="/movies/user/search" class="link"><button class="btn btn-outline-secondarys fs-4 mx-4" style="width: 170px; background-color:#F5D6CB;">유저 검색</button></router-link>
        </div>
        <div class="d-flex row justify-content-center m-5">
          <h4 class="fw-bold">[나를 팔로우 한 친구]</h4>
          <div class="d-flex row justify-content-center" style="max-width:700px;">
            <div id="border" class="mt-3 pt-3 px-4 fs-4 d-flex justify-content-between" style="background-color: #F4F3F3; max-width:700px; height: 70px" v-for="follower in this.followers.followings" :key="follower.id">
              <div>
                <span>친구 이름 : </span><span>{{ follower.username }}</span>
              </div>
              <div>
                <a :href="'http://localhost:8080/home/'+follower.username" class="text-decoration-none">
        ...
        <div class="d-flex row justify-content-center m-5">
          <h4 class="fw-bold">[내가 팔로우 한 친구]</h4>
          <div class="d-flex row justify-content-center" style="max-width:700px;">
            <div id="border" class="mt-3 pt-3 px-4 fs-4 d-flex justify-content-between" style="background-color: #F4F3F3; max-width:700px; height: 70px" v-for="follower in this.followers.followers" :key="follower.id">
              <div>
                <span>친구 이름 : </span><span>{{ follower.username }}</span>
              </div>
              <div>
...
export default {
  ...
  methods: {
    ...mapActions(['fetchProfile', 'fetchCurrentUser', 'fetchFollowers'])
  },
  created() {
